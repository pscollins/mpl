...
struct
  ...
  val primSporkFair' = _prim "spork_fair" : ('aa -> 'ar) * 'aa * ('ba * 'd -> 'br) * 'ba * ('ar -> 'c) * ('ar * 'd -> 'c) * (exn -> 'c) * (exn * 'd -> 'c) -> 'c;
  fun __inline_always__ primSporkFair (body, spwn, seq, sync, exnseq, exnsync) = __inline_always__ primSporkFair' (body, (), spwn, (), seq, sync, exnseq, exnsync)
  ...
  datatype 'a joinpoint = J of { leftSideThread: Thread.t, rightSideThread: Thread.t option ref, tidRight: Word64.word }
  datatype task = GCTask 
  fun push (x): unit = Queue.pushBot (Queue.new()) x
  fun pop (): task option = Queue.popBot (Queue.new())
  fun popDiscard () = case pop () of NONE => false | SOME _ => true
  fun doSpawn (interruptedLeftThread: Thread.t) : unit =
    let
      ...
      val jp = J { leftSideThread = interruptedLeftThread, rightSideThread = rightSideThreadSlot, tidRight = tidRight }
  ...
  fun syncEndAtomic _ (J {tidRight, ...} : 'a joinpoint) : 'b option =
    (if popDiscard () then HH.joinIntoParentBeforeFastClone {thread=Thread.current (), newDepth=1, tidLeft=Word64.fromInt 0, tidRight=tidRight} else (); NONE)
  val sched_package = { syncEndAtomic = syncEndAtomic (fn _ => ()), maybeSpawn = maybeSpawn, returnToSchedEndAtomic = (), assertAtomic = fn _ => fn _ => () }
  fun __inline_always__ tryPromoteNow () = (#maybeSpawn (sched_package) {youngestOptimization=true} (Thread.current ()); ())
  fun __inline_always__ sporkBase (body: unit -> 'a): 'c =
    let
      ...
      fun sync' (bodyr: 'a, jp: int joinpoint): 'c = (#syncEndAtomic (sched_package) jp; raise Die)
      fun exnseq' (e: exn): 'c = raise e
      fun exnsync' (e: exn, jp: int joinpoint): 'c = (#syncEndAtomic (sched_package) jp; raise Die)
    in primSporkFair (body', spwn', seq', sync', exnseq', exnsync') end
  fun __inline_always__ spork {body, spwn, seq, sync, unstolen} = sporkBase body
end
structure ForkJoin0 =
struct
  val spork = Scheduler.spork
  fun par (f, g) = spork { body = f, spwn = g, seq = fn a => (a, g ()), sync = fn ab => ab, unstolen = NONE }
  fun parfor (i, j) f = if i = j then () else (par (fn _ => parfor (i, 0) f, fn _ => parfor (0, j) f); ())
  fun alloc n = let val a = ArrayExtra.Raw.alloc n val _ = if ArrayExtra.Raw.uninitIsNop a then () else parfor (0, n) (fn i => ArrayExtra.Raw.unsafeUninit (a, i)) in ArrayExtra.Raw.unsafeToArray a end
end
